...
class DBWorker:
    ...
    def get_all_materials(self):
        ...
        self.cursor.execute("SELECT m.id_material, m.name_material, m.id_type_material, m.price, m.quantity, m.min_quantity, m.quanity_in_yp, m.ed, mt.name_material, COUNT(mp.count) FROM Material AS m LEFT JOIN MaterialType AS mt ON m.id_type_material = mt.id_type_material LEFT JOIN MaterialProduct AS mp ON mp.id_material = m.id_material GROUP BY m.id_material")
    ...
    def get_all_products(self, id_material: int) -> list:
        try:
            self.cursor = self.connect.cursor()
            self.cursor.execute("SELECT p.id_product, p.name_product, p.article, p.min_price, pt.name_product_type FROM Product AS p LEFT JOIN MaterialProduct AS mt ON p.id_product = mt.id_product AND mt.id_material = %s INNER JOIN ProductType AS pt ON pt.id_product_type = p.id_type_production", (id_material, ))
